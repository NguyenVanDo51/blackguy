...
@endsection
@section('content')
    <h2>Them bai hoc vao trong khóa hoc: {{$course->name}}</h2>
    <div class="w-50 mx-auto mt-3">
        <form method="POST" action="{{route('add-lession', $course->id)}}">
            @csrf()
            <div class="form-group">
                <label for="lession">Số thứ tự (mac dinh la bai ke tiep)</label>
                <input type="number" class="form-control" value="{{ $course->lessions()->max('lession') + 1}}"
                       id="lession" name="lession">
                @error('lession')
            ...
            <a href="{{route('admin', ['function' => 'add-lession', 'course' => $course->id])}}"
